 ...
 */
export async function getResourceUsages(resourceId: string, projectId: string) {
    try {
      const response = await pactum.spec()
      ...
      console.log(`✅ Resource usages fetched successfully for ID: ${resourceId}`);
      return response;
    } catch (error) {
  ...
  export async function updateResourceName(projectId: string, resourceId: string, newName: string) {
    try {
      const response = await pactum.spec()
  ...
  async function getFolderId(resourceId, projectId) {
    const response = await pactum.spec()
      .get(`${BASE_URL}/search`)
      .withQueryParams({ SearchResourceType: "Dataset", SearchTerm: resourceId })
      .withHeaders({ "ProjectId": projectId })
      .expectStatus(200)
  ...
  async function updateResourceFolderId(resourceId, projectId, folderId) {
